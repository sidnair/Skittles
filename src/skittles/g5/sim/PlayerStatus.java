...
public class PlayerStatus 
{
	...
	protected double randEat( int[] aintTemp )			// return the happiness generated by eating, minus is error. -n means eat n colors if n!=1 except n = 1 which means eat more than what in hand
	{
		aintTemp = new int[ aintTemp.length ];
		double dblHappinessUp = 0;
		for ( int intColorIndex = 0; intColorIndex < intColorNum; intColorIndex ++ )
		{
			if ( aintInHand[ intColorIndex ] != 0 )
			{
				intTotalInHand -= aintInHand[ intColorIndex ];
				aintTemp[ intColorIndex ] = aintInHand[ intColorIndex ];
				dblHappinessUp += adblTaste[ intColorIndex ] * Math.pow( aintInHand[ intColorIndex ], 2 );
				aintInHand[ intColorIndex ] -= aintInHand[ intColorIndex ];
				continue;
			}
		}
		dblHappiness += dblHappinessUp;
	...
	public boolean checkCanEat(int[] aintTempEat) 
	{
		...
		if ( intEatColorNum == 1 )
		{
			blnEatOneColor = true;				// Error: either not eating or eat more than one color
		}
		for ( int intColorIndex = 0; intColorIndex < intColorNum; intColorIndex ++ )
		{
